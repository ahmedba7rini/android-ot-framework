...
package com.oneteam.framework.android.location.littlefluffylocation;
...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
  ...
  protected static void processLocation(final Context context, final Location location, final boolean batchResponses, final boolean forceBroadcast) {
      ...
      final float lastSpeed = prefs.getFloat(LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_SPEED, 0F);
      ...
      final float thisSpeed = location.getSpeed();
      ...
      if (!usePreviousReading) {
          ...
          prefsEditor.putFloat(LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_SPEED, thisSpeed);
          prefsEditor.putString(LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_PROVIDER, thisProvider);
          if (LocationLibrary.showDebugOutput) Log.d(LocationLibraryConstants.TAG, TAG + ": Storing location update, lat=" + thisLat + " long=" + thisLong + " accuracy=" + thisAccuracy + " speed=" + thisSpeed + " time=" + LocationInfo.formatTimestampForDebug(thisTime));
      }
      else {
